...
const mutations = {
    ...
    async deleteItem(parent, { id }, ctx, info) {
        ...
        if (!ctx.request.userId && !ctx.request.user) {
            throw new Error("Please sign in before deleting this item!");
        }
        // 1. find the item
    ...
    async signup(parent, args, ctx, info) {
        ...
        console.log(
            "%c resolvers/Mutation.js ",
            "color: #D7B31B; font-weight: bold; font-size: 25px;"
        );
    ...
    async requestReset(parent, args, ctx, info) {
      ...
      <a href="${process.env.FRONTEND_URL}/reset/${resetToken}">Click Here to Reset</a>`),
        ...
        return { message: "Thanks! Check your email soon." };
    },
    ...
    updatePermissions(parent, args, ctx, info) {
        ...
        return hasPermission(currentUser, ["ADMIN", "PERMISSIONUPDATE"])
    ...
    async createOrder(parent, args, ctx, info) {
        ...
        const orderItems = user.cart.map(cartItem => {
            const orderItem = {
                ...
                image: cartItem.item.largeImage,
            };
            delete orderItem.id;
            return orderItem;
        });
